...
const stagingDir = process.env.NODE_ENV === 'production' ? '/run/transcoder' : path.resolve('./');
...
function transcodeRequestHandler(req, res, next) {
  ...
  transcode(file, sizes.split(',')).then(meta => {
...
const app = connect();
app.use('/', transcodeRequestHandler);
app.use(errorHandler);
http.createServer(app).listen(8006);
console.log('Server now listening at http://0.0.0.0:8006');
