...
algorithm to encrypt and decrypt plain utf-8 text
...
KEY = bytes(settings.SECRET_KEY, "utf-8")[:32]
...
def _pad(txt, block_size=16):
    ...
    return (
        txt
        + "\0"
        + "".join([chr(randint(ord("!"), ord("~"))) for x in range(padding - 1)])
    )
...
def decrypt(ciphertext, key=KEY):
    ...
    return plaintext.decode("utf-8").split("\0")[0]
